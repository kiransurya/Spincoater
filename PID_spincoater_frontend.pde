...
void Send_To_Arduino()
{
  ...
  toSend[3] = float(PField.getText());
  toSend[4] = float(IField.getText());// *100000
  toSend[5] = float(DField.getText()); // /10
  Byte a = (AMLabel.getValueLabel().getText()=="Manual")?(byte)0:(byte)1;
  Byte d = (DRLabel.getValueLabel().getText()=="Direct")?(byte)0:(byte)1;
  myPort.write(a);
  myPort.write(d);
...
void serialEvent(Serial myPort)
{
  ...
  if (s.length == 14)
  {
    ...
    if (!pause)
    MyTextarea.append("\nPIDoutput (bMap): "+ LogDisplay3 + " MapedOutput: " + LogDisplay1 +" Kp * Error: " +LogDisplay2 +" Feedback: " + Input + " Setpoint: " + Setpoint+" KI*sampleTime : "+(float(s[5]))+" ITERM (Sum of err): "+LogDisplay4 +" dInput*KD/sampleTime: "+ (float(s[6]) *float(LogDisplay5)) );
